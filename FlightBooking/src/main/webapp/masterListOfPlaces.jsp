...
<title>Master List Of places for source to destination Page</title>
...
<body>
	...
	<div class="container ">
		<h1 class="text-primary">
			Master List Of places for source to destination <a type="button"
				href="masterListOfPlacesInsertNew.jsp"
				class="btn btn-success btn-sm">Add New Source To Destination</a>
		</h1>
	...
	<div class="container mt-3">
		<table class="table table-hover">
			...
			<tbody>
				<%
				List<Source_To_Destination> sdlist = (List<Source_To_Destination>) request.getAttribute("masterListOfPlaces");

				for (Source_To_Destination sd : sdlist) {
				%>

				<tr class="table-light text-dark">
					<th scope="row"><%=sd.getSdid()%></th>
					<td><%=sd.getSource()%></td>
					<td><%=sd.getDestination()%></td>
					<td><form method="post" action="Master_S_To_DList_Controller">
							<button type="submit" value="edit" name="button"
								class="btn btn-warning text-dark">Edit</button>
							<%
							out.print("<input type=hidden name=lblsdid value=" + sd.getSdid() + "> ");
							%>
						</form></td>
